...
var can_shoot = false
const FIREBALL_SCENE = preload("res://Enemies/Piranna Plant/piranna_fire_ball.tscn")
...
func _physics_process(delta: float) -> void:
	...
	var min_angle = base_angle - PI / 3
	var max_angle = base_angle + PI / 3
	target_angle = clamp_angle(target_angle, min_angle, max_angle)
...
func clamp_angle(angle: float, min_angle: float, max_angle: float) -> float:
	...
	var min_wrapped = wrapf(min_angle, -PI, PI)
	var max_wrapped = wrapf(max_angle, -PI, PI)
	return -wrapped
