...
func Physics_Update(delta: float) -> void:
	...
	if Input.is_action_pressed("enter"):
		var speed_factor = clamp(abs(player.velocity.x) / player.yoshi_RUN_MAX_SPEED, 0, 5)
		player.get_current_sprite().speed_scale = speed_factor
		player.velocity.x = move_toward(player.velocity.x, direction * player.yoshi_RUN_MAX_SPEED, player.ACCELERATION * delta)

	else:
		var speed_factor = clamp(abs(player.velocity.x) / player.yoshi_MAX_SPEED, 0, 5)
		player.get_current_sprite().speed_scale = speed_factor
		player.velocity.x = move_toward(player.velocity.x, direction * player.yoshi_MAX_SPEED, player.yoshi_ACCELERATION * delta)
